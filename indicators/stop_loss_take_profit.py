...
class StopLossTakeProfit(bt.Indicator):
    ...
    def __init__(self):
        atr = bt.ind.ATR(self.data, period=self.p.atr_period)
        # By default, plot nothing until an entry is made
        self.lines.stop_loss = float('nan')
        self.lines.take_profit = float('nan')
        self.atr = atr
